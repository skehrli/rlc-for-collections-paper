...
\begin{algorithm}[t]
  ...
  \For{$p \in \mathit{Formals}$} \label{li:init-formals}
...
For notation, \Cref{alg:consistency-checker} assumes that $\mathit{Statement}$
...
\Cref{alg:helpers} checks whether there is some must alias $p \in P$ such that
...
\<@MustCall> methods have already been invoked. \todo{The names CMBefore and
MCBefore are confusing - they're nearly identical. Can we think of better ones
that are equally useful?} If not satisfied, and if $s$ is the exit node
(\cref{li:end-scope}), a violation is reported, as some \<@MustCall> method may
not have been invoked.

If neither of these cases hold, the algorithm propagates the fact to successors
...
continues until $D$ reaches a fixed point.  \todo{Tie this into a running
example?}
