...
is general---it is not specific to resource leaks\footnote{The
  approach described in this section is sufficient to check any
  property of the form ``object O must always call method M before it
  is de-allocated.'' Another such property, for example, is that the
  \<build()> method of a builder~\cite{designpatterns} should always
  be called, to avoid unnecessary computation. We leave applying this
  system to other such properties as future work.}.
