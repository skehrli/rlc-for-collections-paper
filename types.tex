...
\begin{algorithm}[t]
  ...
  \State \textit{// s.  Each fact is of the form } $\langle P, e \rangle$\textit{, where P is a set of variables}
  \State \textit{// and e is an expression with a must-call obligation.}
...
%    \State // \textit{propagate to successors}
 ...
 %   \State // \textit{we even propagate }$\emptyset$\textit{; error reported at exit}
  ...
  \For{each $p \in \mathit{CFG.Formals}$, $t \in \mathit{CFG.succ}(\mathit{CFG.entry})$}\label{li:init-formals}
...
\begin{algorithm}[h]
  ...
  \textsc{MCAfter} and \textsc{CMAfter}, all procedures will be replaced with
  more sophisticated versions in
  \Cref{sec:lightweight-ownership,sec:must-call-choice,sec:reset-must-call}.
  \todo{Just
  merge with other algorithm? In any case ensure they are next to each other.}
}
  \label{alg:helpers}
  \begin{algorithmic}[1]
  ...
  \Procedure{CreatesAlias}{$P,s$}
    \State \Return $s$ is of the form \texttt{p = q} $\wedge\ \mathtt{q} \in P$
  \EndProcedure
  \Procedure{MCAfter}{$p,s$}
  \State \Return \<@MustCall> type of $p$ at program point after $s$
  \EndProcedure
  \Procedure{CMAfter}{$p,s$}
  \State \Return \<@CalledMethods> type of $p$ at program point after $s$
  \EndProcedure
  \end{algorithmic}
...
a set of variables and $e$ is an expression\todo{Why not just store a MustCall type?} \manu{It's useful
for error reporting, and I think that is useful to show even in pseudocode.
Also, I don't think just storing a MustCall type simplifies things much}.  The meaning
of $D$ is as follows: if $\langle P, e \rangle
\in D(s)$, then $e$ has a \<@MustCall> type, and all variables in $P$
...
variables or fields); \textsc{InitialObligations}
...
and \textsc{FactsFromCall} from \Cref{alg:helpers}.  The fixed-point loop then
proceeds by iterating over all facts $\langle P, e \rangle$ present in any
...
\in P$ such that after $s$, the \<@MustCall> type for $p$ is contained in its
...
\begin{figure}
  ...
  \todo{Note where an error is reported.} \manu{Unclear how to do this?  The
  error is detected at the exit, but the ``location'' of the error is node 1.}
...
1: s = makeSocket(addr,port); // new Socket(...) in Java
   if (...) {
2:    s = null;
   } else {
3:    t = s;
