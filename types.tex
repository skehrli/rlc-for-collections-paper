...
\begin{algorithm}[t]
  \caption{Finding unfulfilled \<@MustCall> obligations.
    ...
    \todo{D contains statements and formal parameters as the second element
      ...
      second element of the pair?  Is the use of the AST node to permit
      error reporting?  That detail may or may not be important enough to
      include here in this algorithm.}
...
parameter variable\todo{Why not just store a MustCall type?}.  The meaning
of $D$ is as follows: if $\langle P, e \rangle
\in D(s)$, then $e$ has a non-empty\todo{Is the fact that it is non-empty
  important?  Storing only non-empty ones feels more like an implementation
  optimization.} \<@MustCall> type, and all variables in $P$
