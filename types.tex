...
%    \If{$s$ is \lstinline{p = foo()} and \textsc{HasNonEmptyMustCall}(foo)}
  ...
  \While{$D$ is not at fixed point}
    ...
    \If{\textsc{MCSatisfied}(P,s)}
    \State // \textit{don't propagate to successors}
    \State \textbf{continue} 
    \EndIf
    \If{\textsc{MCMustBeSat}(P,s)}
...
\begin{algorithm}
  \caption{Basic helper functions for \Cref{alg:find-copies}.}
  \begin{algorithmic}[1]
