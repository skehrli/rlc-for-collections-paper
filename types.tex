...
\todo{Put \cref{alg:helpers,alg:consistency-checker} on the same
  page.}
...
\begin{algorithm}[t]
  ...
  \For{$s \in \mathit{Statement}$} \label{li:start-init}
    \For{$v \in \textsc{MCObligations}(s)$}
      \State $\forall t \in \mathit{succ}(s)\ .\ D(t) \leftarrow D(t) \cup \langle \{v\}, \textsc{RHS}(s) \rangle$
...
\begin{algorithm}[h]
  ...
  \todo{Please define every helper function used in this algorithm.  At
    lease give its signature and its specification as a comment, though
    their bodies are not necessary.  I suggest breaking the the list of
    helper methods into two lists:  those that will be redefined in a
    subsequent section, and those that will not.  Each can be in a separate
    algorithm environment.}
  \todo{It's confusing that Procedure MCObligations does not handle code
    like ``myvar = myformal'' where myformal is declared to have a
    \<@MustCall> type.  The current formulation is more syntactic than it
    needs to be.  I suggest permitting the right-hand side to be arbitrary,
    and use the declared type of the right-hand side.  (Why isn't it
    already written like that?)}
}
  \label{alg:helpers}
  \begin{algorithmic}[1]
...
\todo{I think the most important thing to do to make this section more
  comprehensible is to explain the semantics of the domian $D$.  What does
  it represent?  What is
  its relationship to \<@MustCall> types?

Would it be helpful to separate it into two parts, one for obligations and
one for aliases.}
...
high level, the dataflow analysis computes a map $D$ from each statement $s$ to
a \emph{set} of facts of the form $\langle P, e \rangle$, where $P$ is a set of
variables and $e$ is either a method call expression or a formal parameter
variable.  The meaning of $D$ is as follows: if $\langle P, e \rangle \in D(s)$,
then $e$ has a non-empty \<@MustCall> type, and all variables in $P$ are
\emph{must aliases} for the value of $e$ at the program point before $s$.  By
computing $D$, the analysis can find any $e$ that does not have its \<@MustCall>
obligation fulfilled by some must-alias and report an error.
...
% $s'$ is a call statement with\todo{A statement never has a \<@MustCall>
%   obligation.  Only an expression does.}
% a non-empty \<@MustCall> obligation. The variables
% in $P$ are \emph{must aliases} for $s'$\todo{A statement doesn't have
%   aliases of it.  Do you mean of the result of evaluating the statement?
%   or the lhs if the statement is an assignment?}:  hence, satisfying the \<@MustCall>
% obligation for \emph{any} variable in $P$ is sufficient to satisfy the
% obligation for $s'$.  (\Cref{sec:must-call-choice} generalizes the
