...
of the \<Optional> class is unnecessary and complicates the code~\cite{ErnstNothingIsBetterThanOptional}; if \<Optional> was replaced
...
flow-sensitive reasoning or targetting by a \CreatesMustCallFor annotation.
...
due to
\todo{clarify:  ``library incompatibilites'' or ``changed libraries'' or
...
determine that fact).  \Cref{tab:grapple} reports the precision
and recall of the two tools.\todo{The table literally only needs three
...
modeling.\todo{Ask the Grapple authors if there are other reasons for
  false negatives.}\todo{Is another reason that a routine is buggy but the
  bug is not reachable from \<main()>, which is how Grapple works?}
