...
\begin{table}
  ...
  \begin{tabularx}{\columnwidth}{@{}Xr@{}}
    Annotation                           &      Count     \\
    \hline
    \<@Owning> and \<@NotOwning>            &      \todo{113}   \\
    \<@EnsuresCalledMethods>                &      \todo{43}       \\
    \<@MustCall>                            &      \todo{41}       \\
    \<@MustCallAlias>                       &      \todo{36}       \\
    \CreatesObligation                       &      \todo{29}      \\
  \end{tabularx}
\end{table}
...
was caused by the Checker Framework’s overly-conservative
...
We wrote less than one annotation per \todo{2,000} lines of code (\cref{tab:annos}).

\todo{Double-check all numbers and incorporate plume-util code changes.}
We also
...
\todo{Recompute all numbers in table 3.}
...
(except the plume-util micro-benchmark, which barely uses these features).
