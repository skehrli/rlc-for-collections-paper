...
positives (which caused 15\% of the false positives in our case studies) was
a known bug in the Checker Framework’s
type inference algorithm for Java generics~\cite{issue979}.
The second most common reason (causing 10\%)
...
The third most common reason (causing 5\%) is nullness reasoning: some resource
is closed only if it is non-null, but our checker expects the resource to be
closed on every path. Future work could incorporate nullness reasoning to
