...
convenience.\todo{Did we actually choose them because previous work used
  them?  If so, say that.}
\todo{Did we choose the latest commit at the time of our experiments?  Some
  table should give the commit SHAs and dates.}
...
Each case
study took one author a few hours\todo{If it only took a few hours, why
  didn't we add another dozen programs?  We don't necessarily have to
  answer that question explicitly, but reviewers will ask it of themselves
  and may reduce their trust in us as a result.}; most of the time was spent
...
possible resource leaks in each project (``resources'').\todo{What is the
  definition of this?  The static number of resource allocations in the module?}
...
warning.
\todo{Mike suggests deleting the following text.  It sounds bitter and
  petty.  If we want to say anything about this, then fix the bugs and
  re-run the analysis (this should be easy to do), and we can report with
  confidence.  But don't include speculation as the following does.}
