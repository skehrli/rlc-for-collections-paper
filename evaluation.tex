...
%% ----
%% MustCall       13  7   11  31
%% InheritableMC  0   2   7   9   = 40
%% ---
%% MustCallChoice 6   0   28  34
%% PolyMustCall   4   0   0   4   = 38
%% ---
...
the results from their paper.  Unfortunately, we were
not able to successfully run Grapple's leak detection, and full details on their true and false-positive
warnings are currently unavailable, so we cannot comment in detail on the
differences in TP and FP warnings.  Still, the run times in \Cref{tab:grapple}
show a stark difference with our work; \Tool runs in minutes, whereas Grapple
can take many hours.  Further, to our best knowledge, Grapple is not modular, so
any code modification will necessitate a full re-analysis.  \Tool only needs to
re-analyzed modified code and possibly its dependents after a change, not unmodified libraries.
...
\todo{Kill this?} Note that the TP and FP numbers not perfectly comparable between a modular
