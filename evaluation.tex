...
conditions (we plan to add limited support to handle such cases in the future).
...
We ran these analyses on the same versions of our case study programs
that we ran \Tool on in \cref{sec:case-studies}, and examined
...
Further, the analysis is imprecise: in potential leak mode,
the vast majority of warnings are false positives, and even in high-confidence
mode its ratio of true to false positives is higher than \Tool's: (2/9 = 22\%
vs. 45/156 = 29\%).
...
positives than \Tool.  Unfortunately, we could not run
