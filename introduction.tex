...
\todo{I find the following to be misleading.  It's not just an accumulation
  problem.  It is a simple type problem, plus an accumulation problem, plus
  a dataflow analysis.  Because the introduction focuses on just one part
  of our approach, it is surprising to readers when the paper later
  explains all three parts.  The introduction should mention all three
  parts, even if we want to say that the accumulation part is most novel or
  interesting.}
Our
...
checker\todo{Do we want to use the
  term ``must-call checker'' or ``resource leak checker here?}
...
handle:
\todo{I propose combining the first two bullet points into one.  The reason
  is to make this list contain the same number of items as the next list,
  making it obvious which techniques address which problems.  It's a bit
  weird to combine the first two bullet points because it does put two
  unlike things together in the new first bullet point, but the benefit of
  making the paper organization clear to readers is more important, in my opinion.}
\begin{itemize}
...
\item a system for creating new obligations at locations other than the
  ...
  initialization\todo{What is ``multiple initialization''?  Should this be ``re-initialization''?}.
\end{itemize}
...
\item the insight that the resource leak problem is an accumulation
  ...
  of this fact (\cref{sec:base-type-systems}).\todo{Here is another place
    that the introduction is misleading.  Our approach consists of 2 type
    systems (only one of which is an accumulation analysis) plus a dataflow
    analysis, and I think the introduction should make that architecture
    more clear.}
\item three
  % novel
