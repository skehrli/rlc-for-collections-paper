...
every object of a relevant class before it
...
%   interesting.}
Our key insight is that checking must-call properties is an
\emph{accumulation problem}.
Our contribution is a three-phase approach to the resource leak problem that
leverages \emph{accumulation analysis}, satisfying all four requirements:
it is applicable, sound, precise, and fast.
...
Hence, we can build a sound, modular resource leak checker
without doing \emph{any} whole-program alias analysis.
Accumulation also enables a clean design, as we
can factor our checker into a simple core (consisting of a simple
type checker, an accumulation analysis, and a final dataflow analysis), and
then enhance it with targeted alias reasoning to reduce false positives.
...
reasoning about aliasing.  We found several key patterns to
handle:
\begin{itemize}
...
\item the insight that the resource leak problem is an accumulation
  problem, and
  % a novel set of
  a three-phased analysis approach designed to take advantage
