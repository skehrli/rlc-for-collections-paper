...
variables $p_i$ passed to a \CreateObligation target parameter of the method
invoked by $c$.  Then, we can update the \textsc{FactsFromCall} procedure of
\cref{alg:helpers} to handle \CreateObligation as follows:
...
\CreateObligation methods, and the must-call obligation inference ensures the
...
\item any method that re-assigns a non-final, owning field of an object
  must be annotated with an \CreateObligation that targets that object.
...
a \CreateObligation method like \<reconnect()> resets called-methods
