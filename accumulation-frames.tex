...
At each call to a method annotated as \ResetMustCall\<(>\|expr|\<)>, \Tool removes any
...
Since a \ResetMustCall annotation can only increase its target's obligations,
...
\ResetMustCall\<(>\|expr|\<)>,
(1) \Tool\todo{can we be more specific about which component of \Tool?} treats the \MustCall
obligation of \|expr| as \emph{satisfied},
and (2)
the Must Call Consistency Checker creates a fresh obligation to check.
%
In terms of our \cref{sec:must-call-invoked} pseudocode,
assume we have\todo{Put this in the pseudocode} a routine \textsc{COTargets}$(c)$ that returns a set of all
...
\begin{algorithmic}
  ...
  \EndProcedure
  \end{algorithmic}
\todo{Cut or move this text, making all the pseudocode a single \<algorithmic> block.}
I.e., we add one more
...
before the call.
There is one exception to this consistency check: if a \ResetMustCall
...
\ResetMustCall allows \Tool to verify uses of non-final fields
...
\ResetMustCall can also handle cases where object creation
...
% LocalWords:  belowskip MustCall SocketContainer sc ResetMustCall expr
